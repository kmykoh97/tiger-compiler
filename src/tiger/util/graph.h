...
class NodeList {
 public:
  ...
  static NodeList<T>* SubList(NodeList<T>* left, NodeList<T>* right);
  static NodeList<T>* UnionNodeList(NodeList<T>* left, NodeList<T>* right);
};
...
template <class T>
NodeList<T>* NodeList<T>::SubList(NodeList<T>* left, NodeList<T>* right) {
  NodeList<T>* cur = nullptr;

  for(;left;left = left->tail)
  {
    if(!right->InNodeList(left->head))
    {
      cur = new G::NodeList<T>(left->head, cur);
    }
  }
  return cur;
}
...
template <class T>
NodeList<T>* NodeList<T>::UnionNodeList(NodeList<T>* left, NodeList<T>* right) {
  NodeList<T>* cur = left;

  for(;right;right = right->tail)
  {
    if(!left->InNodeList(right->head))
    {
      cur = new G::NodeList<T>(right->head, cur);
    }
  }
  return cur;
}
